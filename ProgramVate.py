...
L = Label(GUI,text='ชื่อสินค้า',font=FONT1).pack() # ข้อความแสดง

v_product = StringVar() # ตัวแปรสำหรับเก็บชื่อสินค้าตอนพิมพ์
E1 = Entry(GUI,textvariable=v_product,font=FONT1)
E1.pack()
#########ช่องกรอกข้อมูล (ราคาสินค้า) ##########
L = Label(GUI,text='ราคาสินค้า',font=FONT1).pack() # ข้อความแสดง

v_price = StringVar() # ตัวแปรสำหรับเก็บราคาสินค้าตอนพิมพ์
E2 = Entry(GUI,textvariable=v_price,font=FONT1)
E2.pack()
#########ช่องกรอกข้อมูล (จำนวน) ##########
L = Label(GUI,text='จำนวน',font=FONT1).pack() # ข้อความแสดง

v_quantity = StringVar() # ตัวแปรสำหรับเก็บจำนวนสินค้าตอนพิมพ์
E3 = Entry(GUI,textvariable=v_quantity,font=FONT1)
E3.pack()
...
R1 = Radiobutton(F1,text='ราคารวม vat แล้ว',variable=v_radio,value='ic')
R1.grid(row=0,column=0)

R1.invoke() #เลือกเป็นค่าเริ่มต้น

R2 = Radiobutton(F1,text='ราคา + vat 7%',variable=v_radio,value='av')
R2.grid(row=0,column=1)

R3 = Radiobutton(F1,text='ราคาไม่รวม vat',variable=v_radio,value='nic')
...
B1 = Button(GUI,text='Calculate',command=Calc)
